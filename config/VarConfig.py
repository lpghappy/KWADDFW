# -*- coding:utf-8 -*-

__author__ = 'boyanru'
__date__ = '2018/03/17'

'''---**---  VarConfig.py 用于定义整个框架中所需要的一些全局常量值 ---**--- '''

import os
# 驱动器路径
ieDriverFilePath = "d:\Python35\IEDriverServer.exe"
chromeDriverFilePath = "d:\Python35\chromedriver.exe"
firefoxDriverFilePath = "d:\Python35\geckodriver.exe"

# 获取当前文件所在目录的父目录的绝对路径
parentDirPath = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
# 异常图片存放目录
screenPicturesDir = parentDirPath + "//exceptionpictures//"

# 测试数据文件存放绝对路径
dataFilePath = parentDirPath + "/testData/126邮箱创建联系人并发送邮件.xlsx"
# dataFilePath = "D:/KeyWordAndDataDrivenFrameWork/testData/126邮箱创建联系人并发邮件.xlsx"

# 测试数据文件中，测试用例表中部分列对应的数字序号
testCase_testCaseName = 1
testCase_frameWorkName = 3
testCase_testStepSheetName = 4
testCase_dataSourceSheetName = 5
testCase_isExecute = 6
testCase_runTime = 7
testCase_testResult = 8

# 用力步骤表中，部分列对应的数字序号
testStep_testStepDescribe = 1
testStep_keyWorks = 2
testStep_locationType = 3
testStep_locatorExpression = 4
testStep_operateValue = 5
testStep_runTime = 6
testStep_testResult = 7
testStep_errorInfo = 8
testStep_errorPic = 9

# 数据源表中，是否执行列对应的数字编号
dataSource_isExecute = 6
dataSource_email = 2
dataSource_runTime = 7
dataSource_result = 8
















