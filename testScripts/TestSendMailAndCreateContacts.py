...
'''---**--- TestSendMailAndCreateContacts.py 用于实现关键字与数据驱动逻辑部分
            登录126邮箱，然后向邮箱中添加联系人并发送一封带附件邮件
   ---**---'''
...
def TestSendMailAndCreateContacts():
    try:
        ...
        for idx, i in enumerate(isExecuteColumn[1:]):
            ...
            if i.value.lower() == "y":
                requiredCase += 1
                # 获取测试用例表中，第idx+1行中用例执行时所使用的的框架类型
                ...
                if useFrameWorkName == "数据":
                    ...
                    # 数据驱动的用例所使用的数据sheet名
                    daraSheetName = excelObj.getCellOfValue(caseSheet, rowNo=idx + 2, colsNo=testCase_dataSourceSheetName)
                    # 获取第idx+1行测试用例的步骤sheet对象
                    stepSheetObj = excelObj.getSheetByName(stepSheetName)
                    print("stepSheetObj %s" %stepSheetObj)
                    # 获取第idx+1行测试用例使用的数据sheet对象
                    dataSheetObj = excelObj.getSheetByName(daraSheetName)
                    print("dataSheetObj %s" %dataSheetObj)
                    # 通过数据驱动框架执行添加联系人
                    ...
                    for index in range(2, stepNums + 1):
                        ...
                        # 获取关键字作为调用的函数名
                        keyWord = stepRow[testStep_keyWorks - 1].value
                        # 获取操作元素定位方式作为调用函数的参数
                        ...
                        try:
                            ...
                            print("执行步骤'%s'发生异常" % stepRow[testStep_testStepDescribe - 1].value)
                            # 截取异常屏幕图片
                            ...
                            successfulSteps += 1
                            print("执行步骤'%s'成功" % stepRow[testStep_testStepDescribe - 1].value)
